...
;(function() {
  ...
  myLib.body.addEventListener('click', function(e) {
    var target = e.target;
    let popupItemClose = myLib.closestItemByClass(target, 'popup-close');
...
;(function() {
  ...
  //   e.preventDefault();
  //   let scrollToItemClass = href.slice(1);
  //   var scrollToItem = document.querySelector('.' + scrollToItemClass);
...
;(function() {
  ...
  var ymapInit = function() {
    ...
    ymaps.ready(function () {
      const ymap = document.querySelector('.contacts__map');
      const coordinates = ymap.getAttribute('data-coordinates');
      const address = ymap.getAttribute('data-address');
...
/* cart start */
;(function(){
  const cartDOMElment = document.querySelector('.js-cart');
  
  if(!cartDOMElment) {
    return;
  }
  const cart = JSON.parse(localStorage.getItem('cart')) || {};
  const cartItemsCounterDOMElement = document.querySelector('.js-cart-total-coutn-items');
  ...
  const renderCartItem = ({id, name, attribute, src, price, quantity}) => {
    ...
    const attributeTemplate = attribute
     ? `<p class="cart-item__attribute">${attribute}</p><input type="hidden" name="${id}-Атрибут" value="${attribute}">`
     : '';
    ...
    <div class="cart-item cart__item">
      ...
      <div class="cart-item__img-wrapper">
        <img src="${src}" alt="" class="cart-item__img">
      </div>
    ...
    cartDOMElment.appendChild(cartItemDOMElement);
  };
  ...
  const deleteCartItem = (id) => {
    const cartItemDOMElement = cartDOMElment.querySelector(`[data-product-id="${id}"]`);
    cartDOMElment.removeChild(cartItemDOMElement);
    delete cart[id];
    updateCart();
  };
